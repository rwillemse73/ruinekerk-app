...
function migrateLegacyExtraInfo(){
 try{
  const raw=localStorage.getItem('ruinekerk_extra_info_v1');
  if(!raw)return;
  const legacy=JSON.parse(raw);
  if(!legacy || typeof legacy!=='object')return;
  let current={};
  try{ current=JSON.parse(localStorage.getItem('ruinekerk_extra_simple_v1')||'{}'); }catch(e){ current={}; }
  ...
  if(changed)localStorage.setItem('ruinekerk_extra_simple_v1',JSON.stringify(current));
