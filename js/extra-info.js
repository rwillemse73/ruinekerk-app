...
function editSimpleExtra(id){
 simpleExtraId=id;
 const item=simpleExtraData()[id];
 if(!item)return;
 simpleExtraEditing=true;
 const x=locations.find(a=>a.id===id);
...
function closeSimpleExtra(){document.getElementById('simpleExtraModal').style.display='none';simpleExtraEditing=false}
...
function renderSimpleExtra(id){
 ...
 const item=simpleExtraData()[id];
 el.innerHTML=item?
  `<div class="meta" style="margin-top:6px"><b>${esc(item.date)}</b></div><div style="margin-top:3px">${esc(item.text)}</div><button type="button" style="margin-top:8px" onclick="editSimpleExtra('${esc(id)}')">✏️ Bewerken</button>`:'';
}
