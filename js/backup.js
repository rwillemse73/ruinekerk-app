/* Reisblik 9.7.5 — Backup lokale gegevens per vakantie */
(function(){
  "use strict";
  const BASE_KEYS=[
  ...
  function make(){
    ...
    return {backupFormatVersion:"2.0",applicationVersion:"9.8.6",backupType:"Reisblik persoonlijke lokale content per vakantie",backupCreatedAt:new Date().toISOString(),activeVacationId:active,vacations:vacations,legacyData:Object.fromEntries(BASE_KEYS.map(x=>[x.key,read(x.key)])),includes:BASE_KEYS.map(x=>x.label),excludes:["Vaste HTML-content","Applicatiebestanden"]};
  }
