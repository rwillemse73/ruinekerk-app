...
(function(){
  ...
  const criteria=[
    {key:'makers',label:'makers/contact',words:['kunstenaar','maker','makers','artist','atelier','ontmoeting','creator','ambacht']},
  ...
  async function search(){
    ...
    // Gebruik alleen parameters die we hier zeker nodig hebben.
    // Ticketmaster markeert latlong inmiddels als deprecated, maar ondersteunt
    // deze parameter nog. De datumselectie doen we bewust lokaal zodat een
    // fout in de array-notatie van startEndDateTime de hele zoekactie niet breekt.
    u.searchParams.set('apikey',key);
    u.searchParams.set('latlong',p.coords.lat+','+p.coords.lon);
    u.searchParams.set('radius',String(p.radius));
    u.searchParams.set('unit','km');
    u.searchParams.set('countryCode','NL');
    u.searchParams.set('size','100');
    u.searchParams.set('sort','distance,asc');
    ...
    try{
      ...
      if(!r.ok){
        ...
        try{
          ...
          detail=clean(errData?.fault?.faultstring||errData?.fault?.detail?.errorcode||'');
        }catch(_){}
        throw new Error('HTTP '+r.status+(detail?' — '+detail:''));
      }
      const data=await r.json();
      const raw=data?._embedded?.events||[];
      const events=raw.map(eventFromApi)
        .filter(x=>Number.isFinite(x.lat)&&Number.isFinite(x.lon))
        .filter(x=>x.date>=p.from&&x.date<=p.to);
      ...
      status.textContent=events.length+' evenementen gevonden binnen '+p.radius+' km voor de gekozen periode.';
      renderResults(events);
    }catch(e){
      console.error('Reisblik Ticketmaster fout:',e);
      status.textContent='Evenementen zoeken mislukt.';
      let message='De externe evenementenbron kon niet worden geraadpleegd.';
      const rawMessage=String(e?.message||'');
      if(rawMessage.includes('HTTP 401')){
        message='Ticketmaster weigert de API-sleutel (401). Controleer of je de Consumer Key hebt gekopieerd en opnieuw op Opslaan hebt geklikt.';
      }else if(rawMessage.includes('HTTP 403')){
        message='Ticketmaster weigert de aanvraag (403). Controleer de status van je applicatie en API-toegang in het Ticketmaster Developer Portal.';
      }else if(rawMessage.includes('HTTP 400')){
        message='Ticketmaster ontvangt een ongeldige zoekopdracht (400). Deze versie gebruikt een vereenvoudigde zoekaanvraag; als dit blijft gebeuren, staat de exacte Ticketmaster-melding hieronder.';
      }else if(rawMessage.includes('Failed to fetch')){
        message='De browser kon Ticketmaster niet bereiken. Controleer de internetverbinding of een browser/netwerkblokkade.';
      }
      box.innerHTML='<div class="agenda-search-help">'+esc(message)+'<br><small>Technische melding: '+esc(rawMessage||'onbekende fout')+'</small></div>';
    }
  }
