// Reisblik 8.3 — daadwerkelijke zoekfunctie
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function renderResults(found, query){
    ...
    status.textContent = found.length + (found.length === 1 ? " resultaat" : " resultaten") + " gevonden voor: “" + query + "”";
  ...
  async function performSearch(){
    ...
    const orphanMatches = [];
    for (const key of PERSONAL_KEYS.slice(0,4)) {
      const data = getPersonalData(key);
    ...
    renderResults(found.concat(orphanMatches), query);
  }
