/* Reisblik 9.2.0 – Mijn reisdag
   Datumselectie, lijst met bezochte locaties en download van de reisdag.
   Word-download gebruikt een echt DOCX-bestand met ingebedde afbeeldingen.
*/
(function(){
  ...
  function imageTypeForMime(mime){
  ...
  async function resizePhotoForDocx(dataUrl,maxWidth=360,maxHeight=360){
    return await new Promise(resolve=>{
      ...
      img.onload=()=>{
        ...
        resolve({dataUrl:resized,width,height});
      };
  ...
  function addTextParagraphs(children,text){
    String(text||'').split(/\n\s*\n/).map(x=>x.trim()).filter(Boolean).forEach(par=>{
      children.push(new docx.Paragraph({text:par}));
    });
  }
  ...
  async function downloadWord(dateFrom,dateTo){
    ...
    if(!window.docx){
      alert('De Word-exportmodule is niet geladen. Probeer het opnieuw.');
      return;
    }
    ...
    try{
      ...
      children.push(new docx.Paragraph({
        text:'Mijn reisdag — '+selectedPeriod,
        heading:docx.HeadingLevel.TITLE
      }));
      children.push(new docx.Paragraph({text:'Samengesteld uit de op deze datum als bezocht geregistreerde locaties.'}));

      for(const [index,entry] of entries.entries()){
        const item=entry.item||{};
        children.push(new docx.Paragraph({
          text:(index+1)+'. '+(item.name||'Onbekende locatie'),
          heading:docx.HeadingLevel.HEADING_1
        }));
        const visited=dateTimePart(item.visitedAt);
        if(visited)children.push(new docx.Paragraph({text:'Bezocht: '+visited}));
        if(item.category)children.push(new docx.Paragraph({text:'Type: '+item.category}));

        // Foto's horen bij de eigen locatie, niet bij het compacte Bezocht-record.
        const sourceLocation=findLocation(item);
        const photo=sourceLocation?.userCreated ? sourceLocation.photo : '';
        if(photo){
          const resizedPhoto=await resizePhotoForDocx(photo,360,360);
          const imageData=resizedPhoto ? dataUrlToUint8Array(resizedPhoto.dataUrl) : dataUrlToUint8Array(photo);
          if(imageData){
            try{
              children.push(new docx.Paragraph({text:'Foto'}));
              const imageWidth=resizedPhoto?.width||360;
              const imageHeight=resizedPhoto?.height||360;
              children.push(new docx.Paragraph({
              ...
              console.warn('Mijn reisdag: foto kon niet in DOCX worden ingebed',e);
            }
          }
        }
        ...
        if(item.category==='evenementen'){
          ...
          if(ev.description)addTextParagraphs(children,ev.description);
          if(ev.location)children.push(new docx.Paragraph({text:'Locatie: '+ev.location}));
          if(ev.organizer)children.push(new docx.Paragraph({text:'Organisator: '+ev.organizer}));
          if(ev.price)children.push(new docx.Paragraph({text:'Prijs: '+ev.price}));
          if(ev.url)children.push(new docx.Paragraph({text:'Evenementwebsite: '+ev.url}));
        }else if(entry.html){
          addTextParagraphs(children,plainTextFromHtml(entry.html));
        }else if(entry.text){
          addTextParagraphs(children,entry.text);
        }else{
          children.push(new docx.Paragraph({text:'Geen tekst beschikbaar in het vaste HTML-bestand.'}));
        }

        if(entry.extra.length){
          children.push(new docx.Paragraph({text:'Extra informatie',heading:docx.HeadingLevel.HEADING_2}));
          entry.extra.forEach(x=>children.push(new docx.Paragraph({
            text:(x.date||'')+(x.date&&x.text?' — ':'')+(x.text||''),
            bullet:{level:0}
          })));
        }
      ...
      const filename=(dateFrom===dateTo?dateFrom:dateFrom+'-tm-'+dateTo)+'-'+getFirstName(matches)+'.docx';
      downloadBlob(blob,filename);
    }catch(e){
      console.error('Mijn reisdag: DOCX export mislukt',e);
      alert('Het Word-bestand kon niet worden gemaakt. Controleer de internetverbinding en probeer opnieuw.');
    }finally{
