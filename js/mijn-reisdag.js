...
(function(){
  ...
  function makeTravelDiaryHtml(entries,dateFrom,dateTo,draft){
    ...
    const cards=entries.map((entry,index)=>{
      ...
      let factual='';
      if(entry.html){
        factual='<div class="dagboek-vaste-tekst">'+entry.html+'</div>';
      }else if(entry.text){
      ...
      const experience=diaryTextBlock('Mijn ervaring',personal,'dagboek-ervaring');
      const noteBlock=diaryTextBlock('Mijn notitie',note,'dagboek-notitie');
  ...
  function travelDiaryCss(){
    return `
      *{box-sizing:border-box}
      ...
      .dagboek-header{background:#e8df73;border-top:4px solid #1e292d;border-bottom:4px solid #1e292d;text-align:center;padding:25px 28px 22px;margin-bottom:18px}
      .dagboek-label{font:700 11px/1.2 Arial,sans-serif;letter-spacing:3px;margin-bottom:10px;color:#354247}
      ...
      .dagboek-hero figure{position:relative;margin:0;min-width:0}
      .dagboek-hero img{display:block;width:100%;height:300px;object-fit:cover}
      ...
      .dagboek-bezocht{display:flex;align-items:center;gap:12px;background:#c7e1df;padding:11px 16px;margin:0 0 22px;border-top:1px solid #a9c9c7;border-bottom:1px solid #a9c9c7}
      .dagboek-pin{font:22px Arial,sans-serif}
      .dagboek-bezocht strong{font-size:28px;letter-spacing:1px}
      .dagboek-bezocht em{margin-left:auto;font-size:20px}
      .dagboek-grid{display:grid;grid-template-columns:1fr 1fr;gap:18px;align-items:start}
      .dagboek-kaart{break-inside:avoid;border:1px solid #ddd4ca;background:#fcfaf5;overflow:hidden}
      .dagboek-kaart-kop{display:flex;background:#e8d2cb;min-height:54px;align-items:stretch}
      .dagboek-nummer{display:flex;align-items:center;justify-content:center;background:#1e292d;color:#fff;font:bold 22px Arial,sans-serif;width:52px;flex:0 0 52px}
      .dagboek-kaart-titel{padding:8px 10px 7px;min-width:0}
      .dagboek-kaart-kop h2{font-size:22px;line-height:1.08;margin:0 0 4px}
      .dagboek-meta{font:12px/1.3 Arial,sans-serif;color:#5e686b}
      .dagboek-kaart-inhoud{display:grid;grid-template-columns:1fr 1fr;gap:12px;padding:12px 12px 0}
      .dagboek-foto{margin:0;position:relative;min-width:0}
      .dagboek-foto img{display:block;width:100%;height:190px;object-fit:cover}
      .dagboek-feit{min-width:0}
      .dagboek-feit h3,.dagboek-blok h3{font:bold 11px/1.2 Arial,sans-serif;letter-spacing:1.5px;margin:0 0 7px;color:#3d4b50}
      .dagboek-vaste-tekst{font-size:15px;line-height:1.45}
      .dagboek-vaste-tekst p{margin:0 0 9px}
      ...
      .dagboek-ervaring,.dagboek-notitie{margin:12px;background:#dce8df;padding:10px 12px}
      .dagboek-ervaring p,.dagboek-notitie p{font-size:15px;margin:0}
      .dagboek-notitie{background:#eee8d7}
      .dagboek-afsluiting{display:flex;gap:12px;align-items:center;border-top:2px solid #26353a;border-bottom:1px solid #c9c2b8;margin:28px 0 18px;padding:13px 4px}
      .dagboek-afsluiting>span{font-size:28px}.dagboek-afsluiting strong{font:bold 18px Arial,sans-serif;letter-spacing:1.5px}.dagboek-afsluiting p{margin:2px 0 0;font-style:italic;font-size:16px}
      .dagboek-leeg{text-align:center;padding:30px}
      .dagboek-footer{text-align:center;padding:15px;font-style:italic;font-size:17px;border-top:1px solid #26353a}
      @media(max-width:760px){
        .dagboek-pagina{padding:14px 12px 30px}
        .dagboek-header{padding:20px 14px 17px}.dagboek-header h1{font-size:39px}.dagboek-header h2{font-size:22px}
        .dagboek-hero{grid-template-columns:1fr;gap:10px}.dagboek-hero img{height:230px}
        .dagboek-bezocht{margin-bottom:14px}.dagboek-bezocht strong{font-size:24px}.dagboek-bezocht em{font-size:15px}
        .dagboek-grid{grid-template-columns:1fr;gap:14px}
        .dagboek-kaart-kop h2{font-size:20px}
        .dagboek-kaart-inhoud{grid-template-columns:1fr;padding:10px 10px 0}.dagboek-foto img{height:auto;max-height:none}
        .dagboek-feit{padding-top:2px}.dagboek-vaste-tekst{font-size:16px}
      }
      @media print{body{background:#fff}.dagboek-pagina{width:auto;padding:10mm}.dagboek-header,.dagboek-bezocht,.dagboek-kaart-kop,.dagboek-ervaring,.dagboek-notitie{-webkit-print-color-adjust:exact;print-color-adjust:exact}.dagboek-grid{gap:12px}.dagboek-kaart{break-inside:avoid}}
    `;
  }
  ...
  function makeTravelDiaryHtmlWithDraft(entries,dateFrom,dateTo,draft){
    const base=makeTravelDiaryHtml(entries,dateFrom,dateTo);
    const doc=new DOMParser().parseFromString(base,'text/html');
    if(draft?.title)doc.querySelector('.dagboek-header h1').textContent=draft.title;
    const intro=doc.querySelector('.dagboek-intro');
    if(intro)intro.textContent=draft?.intro||'';
    if(intro && !draft?.intro)intro.remove();
    (draft?.items||[]).forEach((d,i)=>{
      const article=doc.querySelectorAll('.dagboek-locatie')[i];
      if(!article)return;
      if(d.story){
        let block=article.querySelector('[data-personal-draft]');
        if(!block){block=doc.createElement('div');block.className='dagboek-blok';block.setAttribute('data-personal-draft','1');article.appendChild(block);}
        block.innerHTML='<h3>Mijn ervaring</h3>'+String(d.story).split(/\n\s*\n/).filter(Boolean).map(p=>'<p>'+escapeHtml(p).replace(/\n/g,'<br>')+'</p>').join('');
      }
      if(d.note){
        let block=article.querySelector('[data-note-draft]');
        if(!block){block=doc.createElement('div');block.className='dagboek-blok';block.setAttribute('data-note-draft','1');article.appendChild(block);}
        block.innerHTML='<h3>Mijn notitie</h3>'+String(d.note).split(/\n\s*\n/).filter(Boolean).map(p=>'<p>'+escapeHtml(p).replace(/\n/g,'<br>')+'</p>').join('');
      }
    });
  ...
  async function generateDiaryHtml(entries,dateFrom,dateTo,draft){
    const html=makeTravelDiaryHtmlWithDraft(entries,dateFrom,dateTo,draft);
  ...
  function showDiaryEditor(entries,dateFrom,dateTo){
    const existing=byId('reisdagEditor');
    if(existing){existing.scrollIntoView({behavior:'smooth',block:'start'});return;}
    const panel=ensureResultsPanel();
    const wrapper=document.createElement('div');
    wrapper.innerHTML=makeDiaryEditorHtml(entries,dateFrom,dateTo);
    panel.appendChild(wrapper.firstElementChild);
    byId('diarySaveBtn').onclick=()=>{
      const draft=collectDiaryDraft(entries,dateFrom,dateTo);
      saveDiaryDraft(dateFrom,dateTo,draft);
      const status=byId('diarySaveStatus');
      if(status)status.textContent='✓ Opgeslagen op dit apparaat voor deze vakantie en datum.';
    };
    byId('diaryGenerateBtn').onclick=async()=>{
      const draft=collectDiaryDraft(entries,dateFrom,dateTo);
      saveDiaryDraft(dateFrom,dateTo,draft);
      await generateDiaryHtml(entries,dateFrom,dateTo,draft);
      const status=byId('diarySaveStatus');
      if(status)status.textContent='✓ Opgeslagen en HTML gegenereerd.';
    };
    byId('diaryCancelBtn').onclick=()=>wrapper.remove();
    byId('reisdagEditor').scrollIntoView({behavior:'smooth',block:'start'});
  }
  ...
  function showResults(dateFrom,dateTo){
    ...
    edit.id='reisdagEditBtn';
    edit.type='button';
    edit.textContent='✏️ Reisdagboek bewerken';
    edit.onclick=async()=>{
      const entries=await buildDayData(dateFrom,dateTo);
      showDiaryEditor(entries,dateFrom,dateTo);
    };
