...
(function(){
  ...
  function showSelectedLocationsOnMap(selectedDate){
    const overlay=byId('reisdagMapOverlay');
    ...
    if(!points.length){
      alert('Voor '+selectedDate+' zijn geen geselecteerde locaties met GPS-coördinaten gevonden.');
      return;
    }

    overlay.style.display='flex';
    const title=byId('reisdagMapTitle');
    if(title)title.textContent='🗺️ Mijn reisdag — '+selectedDate;
    ...
    if(bounds.length===1){
      reisdagMapInstance.setView(bounds[0],16);
    }else{
      reisdagMapInstance.fitBounds(bounds,{padding:[30,30],maxZoom:16});
    }
  ...
  function showResults(selectedDate){
    ...
    word.onclick=()=>downloadWord(selectedDate);

    const map=document.createElement('button');
    map.id='reisdagMapBtn';
    map.type='button';
    map.textContent='🗺️ Toon op kaart';
    map.onclick=()=>showSelectedLocationsOnMap(selectedDate);
