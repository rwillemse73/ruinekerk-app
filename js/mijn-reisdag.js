...
(function(){
  ...
  async function resizePhotoForDocx(dataUrl,maxWidth=220,maxHeight=260){
  ...
  async function downloadWord(dateFrom,dateTo){
    ...
    try{
      ...
      for(const [index,entry] of entries.entries()){
        ...
        if(photo){
          const resizedPhoto=await resizePhotoForDocx(photo,220,260);
          ...
          if(imageData){
            try{
              ...
              const imageWidth=resizedPhoto?.width||220;
              const imageHeight=resizedPhoto?.height||260;
              children.push(new docx.Paragraph({
