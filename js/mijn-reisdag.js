...
(function(){
  ...
  function japanPrepareLocationHtml(rawHtml,locationName,locationIndex) {
    ...
    try {
      ...
      // 9.7.22: iedere locatie krijgt standaard twee extra, handmatig te vullen
      // fotoplaatsen. De eerste staat direct onder de <h2> en vult de kolom;
      // de tweede staat midden in de tekst en zweeft rechts zodat de tekst eromheen loopt.
      root.querySelectorAll('.tekst-normaal').forEach((blok, blokIndex) => {
        const h2 = blok.querySelector(':scope > h2');
        const full = blok.ownerDocument.createElement('img');
        ...
        else blok.insertBefore(full, blok.firstChild);

        const inline = blok.ownerDocument.createElement('img');
        ...
        // Plaats de tweede foto na de eerste inhoudelijke alinea.
        const paragraphs = [...blok.querySelectorAll(':scope > p')];
        if (paragraphs.length) {
          paragraphs[0].insertAdjacentElement('afterend', inline);
        } else {
          const firstBlock = [...blok.children].find(el => el !== h2 && el !== full);
          if (firstBlock) firstBlock.insertAdjacentElement('afterend', inline);
          else blok.appendChild(inline);
        }
      });
...
<dialog id="middag-avond">
  ...
  async function makeJapanTravelHtml(entries,dateFrom,dateTo) {
    ...
    const columnHtml=(items,columnNumber)=>items.map(entry=>`    <div class="tekst-normaal">
${entry.html}
    </div>`).join('\n')+
