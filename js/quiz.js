...
(function(){
  ...
  function uniqueDistractors(f, facts, count=3){
    ...
    const candidates=facts
      ...
      .filter(x=>x.topic===topic || topic==='bijzonder' || x.topic==='bijzonder')
      .filter(x=>x.text.length>=CONFIG.minDistractorLength)
      .map(x=>({text:shorten(x.fullText), fact:x}))
      .filter(x=>normalize(x.text)!==normalize(f.fullText));
  ...
  function questionFromFact(f){
    ...
    if(years.length){
      const year=years[0];
      if(/\b(verwoest|vernield|gebouwd|gesticht|opgericht|geopend|gesloten|overleden|geboren|aangelegd|hersteld|gerestaureerd|geschonken|verkocht|gekocht|verplaatst|ontstaan|gebeurde)\b/i.test(text)){
        question=`Wat gebeurde er in ${year} bij ${name}?`;
      }else{
        question=`Welk jaartal hoort bij deze gebeurtenis in ${name}?`;
      }
      return question;
    }

    if(/\b(ligt|gelegen|bevindt zich|staat|is gelegen)\b/i.test(text)){
      return `Waar bevindt ${name} zich volgens de beschreven locatie?`;
    }
    if(/\b(landschap|omgeving|natuur|bos|heide|rivier|meer|gebied|hoogteverschil|stuwdam)\b/i.test(text)){
      return `Wat kenmerkt het landschap of de omgeving van ${name}?`;
    }
    if(/\b(kasteel|kerk|huis|buitenplaats|landgoed|toren|monument|ruïne|fort|gebouw)\b/i.test(text)){
      return `Welke bijzonderheid over ${name} wordt beschreven?`;
    }
    if(/\b(schonk|erfde|bewoonde|eigenaar|stichter|bewoner|familie|zoon|dochter|graaf|hertog|baron|koning|persoon)\b/i.test(text)){
      return `Welke persoon of familie wordt in verband gebracht met ${name}?`;
    }
    if(/\b(oorlog|bombardement|strijd|beleg|verwoesting|geschiedenis|eeuw|middeleeuwen)\b/i.test(text)){
      return `Wat gebeurde er in de geschiedenis van ${name}?`;
    }
    if(/\b(kunst|cultuur|legende|verhaal|traditie|religie)\b/i.test(text)){
      return `Welke culturele of historische bijzonderheid hoort bij ${name}?`;
    }
    return `Welke beschrijving past bij ${name}?`;
  }
