/* Reisblik 9.0.4 – Agenda: permanente agenda + handmatige JSON-import + ontdubbeling + archief */
(function(){
  ...
  async function importAgendaJson(){
    ...
    try{
      const response=await fetch(importUrl()+'?v=9.0.4&t='+Date.now(),{cache:'no-store'});
      ...
      incoming.forEach(event=>{
        const normalized=normalizeEvent(event,{file:payload.sourceFile||importUrl()});
  ...
  function handleVacationChange(){importedEvents=[];const status=el('agendaImportStatus');if(status)status.textContent='';renderAgenda();}
