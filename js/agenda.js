/* Reisblik 8.8.6 – Agenda / lokaal bewaren + automatische testimport */
(function(){
  ...
  async function importTestAgenda(){
    try{
      const response=await fetch(TEST_IMPORT_URL+'?v=8.8.6',{cache:'no-store'});
      ...
      incoming.forEach(event=>{
        ...
        events.push({
          id:event.id||null, name:String(event.name||'').trim(), date:String(event.date||'').trim(),
          location:String(event.location||'').trim(), lat:Number(event.lat), lon:Number(event.lon),
          type:String(event.type||'').trim(), organizer:String(event.organizer||'').trim(),
          price:String(event.price||'').trim(), description:String(event.description||'').trim(),
          source:String(event.source||'').trim(), url:String(event.url||'').trim(),
          reisblikMatch:Number(event.reisblikMatch)||0, matchReasons:Array.isArray(event.matchReasons)?event.matchReasons:[],
          importedFrom:'agenda-test.json', importedAt:new Date().toISOString(), savedAt:new Date().toISOString()
        });
        keys.add(key); added++;
      });
      ...
      console.warn('Agenda testimport niet uitgevoerd:',error);
      return 0;
    }
  }
  ...
  function openAgenda(){
    ...
    renderSavedEvents();
    importTestAgenda().then(function(){ renderSavedEvents(); });
  ...
  window.reisblikAgendaGetEvenementen=function(){ return loadEvents().slice(); };
