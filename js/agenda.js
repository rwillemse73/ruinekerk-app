/* Reisblik 9.1.1 – Agenda: versieherkenning + categorie-indeling bij import, behoud bestaande functionaliteit */
(function(){
  const SAVED_BASE_KEY='reisblik_agenda_evenementen_v1';
  function storageKey(){return window.reisblikVakantie?.getVakantieStorageKey ? window.reisblikVakantie.getVakantieStorageKey(SAVED_BASE_KEY) : SAVED_BASE_KEY;}
  function importUrl(){
  ...
  async function importAgendaJson(){
    ...
    try{
      const response=await fetch(importUrl()+'?v=9.1.1&t='+Date.now(),{cache:'no-store'});
