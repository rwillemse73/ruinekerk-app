/* Reisblik 9.3.6 – snellere locatie-load + robuuste HTML-paden */
let locations=[];
...
async function loadFixedHtmlLocations(){
 ...
 // 9.3.6: never derive all HTML files from locaties.json during vacation load.
 // The category JSON supplies the location records; HTML is loaded on demand.
 if(!Array.isArray(configured) || !configured.length)return result;
 for(const [i,file] of configured.entries()){
  const rawPath=file?.path||file;
  if(typeof rawPath!=='string' || !/\.html$/i.test(rawPath))continue;
  const id=file?.id || rawPath.replace(/\.html$/i,'').split('/').pop() || ('html-'+i);
  const candidate=base && rawPath.startsWith(base+'/') ? rawPath : (base?base+'/'+rawPath:rawPath);
  try{
   ...
   d.id=id; d.content=response.url.split('?')[0]; d.userCreated=false; result.push(d);
  }catch(e){console.error('Kon '+candidate+' niet laden',e);}
 }
 return result;
}
...
function contentUrlCandidates(x){
 ...
 const raw=String(x.content).replace(/^\.\//,'');
 const base=window.reisblikVakantie?.getActieveVakantieBasePath ? window.reisblikVakantie.getActieveVakantieBasePath() : '';
 const cat=userLocationCategory(x); const out=[];
 const add=value=>{ if(!value)return; try{const u=new URL(value,location.href).href;if(!out.includes(u))out.push(u);}catch(e){} };
 // Exact JSON path first.
 add(raw);
 if(base && !/^(https?:)?\/\//i.test(raw) && !raw.startsWith('/') && !raw.startsWith('../')){
  if(raw===base || raw.startsWith(base+'/'))add(raw);
  else if(raw.toLowerCase().startsWith(cat.toLowerCase()+'/'))add(base+'/'+raw);
  else add(base+'/'+cat+'/'+raw.replace(/^\//,''));
 }
 // Old Praag sublocation paths are now stored in praag/locaties/.
 for(const u of [...out])if(/\/sublocaties\//i.test(u))add(u.replace(/\/sublocaties\//ig,'/locaties/'));
 // Legacy bare filename fallback.
 const filename=raw.split('/').pop();
 if(filename && /\.html$/i.test(filename)){
  if(base)add(base+'/'+cat+'/'+filename);
  add(cat+'/'+filename);
  if(base)add(base+'/locaties/'+filename);
  add('locaties/'+filename);
 }
 return out;
}

async function fetchLocationHtml(x,cacheLabel='content'){
 const candidates=contentUrlCandidates(x); let lastResponse=null,lastError=null;
 for(const url of candidates){
  try{
   const response=await fetch(url+(url.includes('?')?'&':'?')+cacheLabel+'='+Date.now(),{cache:'no-store'});
   lastResponse=response; if(response.ok)return response;
  }catch(e){lastError=e;}
 }
 if(lastResponse)throw new Error('HTTP '+lastResponse.status);
...
async function readOpeningHoursFromHtml(x){
 ...
 try{
  const response=await fetchLocationHtml(x,'openhours');
...
// 9.3.6: loadStory resolves HTML paths on demand; no preload wrapper needed.
loadStory=async function(x,clicked){ return originalLoadStory(x,clicked); };
