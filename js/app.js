/* Reisblik 9.5 – Eigen locaties + onderhoudsversie */
let locations=[];
...
async function loadFixedHtmlLocations(){
 ...
 // Only explicitly configured fixed HTML locations are loaded here.
 // Do NOT derive all locations from locaties.json: normal location content
 // is loaded on demand when the user opens a location.
 const files=(configured.length ? configured : []).map((file,i)=>({
