/* Reisblik 9.3.5 – HTML-paden robuust oplossen */
let locations=[];
...
async function loadFixedHtmlLocations(){
 ...
 for(const f of files){
  const probe={id:f.id,content:f.file,category:'locaties'};
  try{
   const fetched=await fetchLocationHtml(probe);
   const response=fetched.response;
   const path=fetched.url;
   ...
   d.userCreated=false;
   result.push(d);
  }catch(e){console.error('Kon HTML voor '+(f.file||f.id)+' niet laden',e);}
 }
 return result;
}
...
function contentUrlCandidates(x){
 const out=[];
 if(!x || !x.content)return out;
 const raw=String(x.content).trim().replace(/^\.\//,'');
 const base=window.reisblikVakantie?.getActieveVakantieBasePath ? String(window.reisblikVakantie.getActieveVakantieBasePath()||'').replace(/^\/+|\/+$/g,'') : '';
 ...
 // Already complete paths are tried first.
 add(clean);

 // Older Praag data can contain praag/sublocaties/. The current GitHub
 // structure keeps main and sublocation HTML together in praag/locaties/.
 if(/(^|\/)sublocaties\//i.test(clean)){
  add(clean.replace(/(^|\/)sublocaties\//i,'$1locaties/'));
 }

 // Add the active vacation prefix without duplicating it.
 if(base){
  if(clean.startsWith(base+'/')) add(clean);
  else add(base+'/'+clean);
 }

 // Also try category-relative and shared locaties folders.
...
async function fetchLocationHtml(x, queryKey='v'){
 const candidates=contentUrlCandidates(x);
 let lastError=null;
 for(const url of candidates){
  try{
   const response=await fetch(url+'?'+queryKey+'='+Date.now(),{cache:'no-store'});
   if(response.ok)return {response,url};
   lastError=new Error('HTTP '+response.status+' voor '+url);
  }catch(e){lastError=e;}
 }
 throw (lastError || new Error('Geen geldig HTML-pad gevonden voor '+(x?.name||x?.id||'locatie')));
}

async function readOpeningHoursFromHtml(x){
 try{
  const fetched=await fetchLocationHtml(x,'openhours');
...
loadStory=async function(x,clicked){
 if(!x.userCreated && x.content){
  ...
  for(const url of candidates){
   x.content=url;
   try{
    await originalLoadStory(x,clicked);
    const el=document.getElementById('story-'+x.id);
    if(el && !el.textContent.includes('Het locatieverhaal kon niet worden geladen.')){
     return;
    }
   }catch(e){
    console.warn('HTML-pad mislukt voor '+x.name+': '+url,e);
   }
  }
  x.content=oldContent;
  const el=document.getElementById('story-'+x.id);
  if(el)el.textContent='Het locatieverhaal kon niet worden geladen.';
  return;
 }
